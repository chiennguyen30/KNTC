...
const QLNamHoc = (props) => {
    ...
    const showModalEdit = (id) => {
        ...
        setAction('edit');
        api.ChiTietThamQuyen({ ThamQuyenID })
            .then((res) => {
                console.log(res);
                if (res.data.Status > 0) {
    ...
    const renderThaoTac = (record) => {
        return (
            <div className={'action-btn'}>
                ...
                {/* {role.delete ? */}
                <Tooltip title={'Xóa'}>
                    <DeleteOutlined onClick={() => deleteModalAddEdit(record.ThamQuyenID)} />
                </Tooltip>
                {/* : ""} */}
            </div>
        );
