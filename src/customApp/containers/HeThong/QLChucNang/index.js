  ...
  const renderThaoTac = (record) => {
    return (
      <div className={"action-btn"}>
        ...
        {/* {role.delete ? */}
        <Tooltip title={"Xóa"}>
          <DeleteOutlined onClick={() => deleteModalAddEdit(record.ChucNangID)} />
        </Tooltip>
        {/* : ""} */}
      </div>
    );
  ...
  const columns = [
    {
      ...
      render: (text, record, index) => {
        return record.ChucNangChaID === null ? <b>{record.TT}</b> : <p>{record.TT}</p>;
      },
    },
    {
      title: "Mã chức năng",
    ...
    },
    {
      ...
      width: "10%",
      render: (text, record, index) => {
        return record.ChucNangChaID === null ? <b>{record.Icon}</b> : <p>{record.Icon}</p>;
      },
    },
    {
