...
export default (props) => {
  ...
  console.log("chien123", getAllChucNang);
  ...
  return (
    ...
    >
      <Form form={form} name={"formhuongdan"}>
        ...
        <Item label="Chức năng" name={"ChucNangID"} {...ITEM_LAYOUT} rules={[{ ...REQUIRED }]}>
          <Select onChange={handleChange} placeholder="Chọn chức năng">
            {getAllChucNang.map((record, index) => {
              return (
                <Option key={index} disabled={!record.Highlight} value={record.ChucNangID}>
                  {record.TenChucNang}
                </Option>
              );
            })}
        ...
        <Item label="Tiêu đề hướng dẫn" name={"TieuDe"} {...ITEM_LAYOUT} rules={[{ ...REQUIRED }]}>
          <Input />
        </Item>
        {action === "edit" ? (
          ...
          <Item label="File Hướng dẫn" name={"importData"} {...ITEM_LAYOUT} rules={[REQUIRED]}>
            <StyledImport>
              <Upload {...uploadFile}>
                <Button type="primary" icon={<FileAddOutlined />}>
                  Chọn file
        ...
        <StyledColor>
          <p>
            Lưu ý : File hướng dẫn sử dụng yêu cầu file có định dạng .pdf và có dung lượng &#60; 10
            MB
          </p>
        </StyledColor>
      </Form>
