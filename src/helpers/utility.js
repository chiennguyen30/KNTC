...
export function handleDate(
    dateString,
    formatsDate = ["YYYY-MM-DD", "DD/MM/YYYY"]
) {
    if (!dateString) return null;
    let newDateString = dateString.split("T")[0];
    let dateObj = dayjs(newDateString, formatsDate, true);

    if (!dateObj.isValid()) dateObj = null;
...
export function getFilterData(oldFilterData, onFilter, onOrder) {
    ...
    if (
        (filterData.PageNumber === 1 &&
            filterData.PageSize === DefaultPageSize) ||
        !filterData.PageNumber
    ) {
...
export const handleTextLong = (text, lengthText = 20) => {
...
export function beforeUpload(file) {
    ...
    if (!isLimit) {
        message.error(
            "Dung lượng file " +
                file.name +
                " đính kèm quá lớn, không thể tải lên"
        );
    }
    return isLimit;
}
...
export const formatTreeDataKhieuTo = (Data) => {
    ...
    const renderChildrenTreeSelect = (children, indexRoot) => {
        return children.map((value, index) => {
            ...
            let children =
                value.DanhMucLoaiKhieuToCon &&
                value.DanhMucLoaiKhieuToCon.length > 0
                    ? renderChildrenTreeSelect(
                          value.DanhMucLoaiKhieuToCon,
                          index
                      )
                    : null;
